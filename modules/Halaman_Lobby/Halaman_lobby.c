...
int get_terminal_width(char width){ 			//t = tinggi, l = lebar
    CONSOLE_SCREEN_BUFFER_INFO csbi;			//struktur data windows API untuk mengakses data seputar layar tampilan
  	
	if (width == 'l'){	//jika memilih lebar
    	int lebar;
	    if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi)) {		//struktur data windows API untuk memperoleh data seputar layar tampilan
	        lebar = csbi.srWindow.Right - csbi.srWindow.Left + 1;						//mendapatkan lebar
	    } else {
	        lebar = 80; // Default jika gagal
	    }
	    return lebar;
	}else if (width == 't'){	//jika memilih tinggi
		int tinggi;
		if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi)) {		//struktur data windows API untuk memperoleh data layar tampilan
	    	tinggi = csbi.srWindow.Bottom - csbi.srWindow.Top + 1;						//mendapatkan tinggi
	    } else {
